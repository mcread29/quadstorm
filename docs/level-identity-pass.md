...
This document defines the remaining generation slice for the game map. The full-level overview, three small-map systems fixtures, replayable new-match boundary, and first physically larger Fortress V1 profile are complete. Normal generation now combines radius 8 with `worldScale = 0.22` and actor-relative acceptance rather than merely increasing hex radius. Broader topology, room-shape grammar, districts, semantic quest anchors, opening-component validation, and pacing remain because scale alone cannot make a structurally repetitive map compelling.
...
Crowd navigation, local separation, deterministic spawn pacing, enemy roles, the endless director, random large-map generation, and first physical scaling are complete. The next work should establish broader topology/room identity, quest-aware semantic anchors and binding, opening-component validation, and larger-map pacing before adding deeper clue families.
