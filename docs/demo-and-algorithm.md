...
The separate 2.5D runtime is tracked in [`game-roadmap.md`](game-roadmap.md), with continuation details in [`game-handoff.md`](game-handoff.md). It wraps the generation chain in a replayable match-seed boundary with bounded **Fortress V1** candidate retries, same-map restart, distinct New Match, actor-relative physical validation, and visible radius-5 fixture fallback. The next target dynamically binds compatible quest recipes and expands topology/room-shape identity beyond the current spatial-tree planner. The hard-coded deterministic arena remains available through `F1` as a separate regression wrapper.
...
The default **shooter layout** starts with an abstract mission graph. It anchors start and exit arenas near well-separated selected boundary entrances, distributes additional arena seeds using farthest-point sampling, and grows compact combat rooms around them while retaining negative space. Five-arena systems maps use their selected small recipe; larger maps select one of five validated graph archetypes; compact four-arena layouts retain a noise-perturbed Prim-like spatial tree. Each planned edge is routed through unoccupied cells with physical costs that penalize low clearance and narrow portals. Longer routes become separate connector rooms and gain lateral cells where space allows. Links shorter than an arena's approximate diameter are folded into an endpoint arena and become direct arena doorways, avoiding a separate tiny connector for every mission-graph edge. At least one route remains an explicit connector, while additional connector identities are reserved for long passages. Opportunistic widening does not mathematically guarantee that every connector is narrower than every arena. Only planned arena/corridor contacts become logical doorways, so incidental physical contact cannot introduce an unintended shortcut.

That pipeline is not the endpoint for map identity. Radius-5 maps select Hub Circuit, Broken Ring, or Twin Wings before placement and remain regression fixtures. Normal Fortress V1 generation now uses radius 8, `worldScale = 0.22`, the larger spatial planner, one generated Hub, and a leaf-preferred Reward arena; actor-relative metric gates reject cramped or under-capacity candidates. The remaining production pass must publish puzzle-driven room shapes, districts, additional topology/quest recipes, and semantic anchors, then reject candidates that fail deeper endurance or quest constraints. See [`level-identity-pass.md`](level-identity-pass.md).
...
- Deeper physical pacing for the radius-8, `0.22F` Fortress V1 profile; movement, projectiles, interactions, lighting, detail density, and navigation performance still need tuning.
- Large-map crowd/navigation scaling beyond the initial map-wide ingress and spawn-capacity gates.
