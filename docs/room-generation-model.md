...
> Runtime status: `stalberg_game` consumes the complete generation chain through immutable `GeneratedLevel`, renders exact assigned floors, retains exact doorway thresholds, and publishes door-aware navigation. `LevelSession` owns the authoritative player and synchronized collision/traversal locks. `HordeMatch` runs one persistent radius-5 map with points, permanent gates, upgrades, five deterministic rounds, door-aware Drifter/Runner/Caster/Elite enemies, an Anchor holdout, Hub activation, an optional ordered relay reward, and explicit Exit completion. Small shooter layouts publish Hub Circuit, Broken Ring, or Twin Wings before candidate placement; room-shape grammar and broader recipes remain in [`level-identity-pass.md`](level-identity-pass.md). Regression `CombatState`, the preserved generated-encounter modules, and the hard-coded `F1` arena remain foundations; see [`small-puzzle-horde-slice.md`](small-puzzle-horde-slice.md) for acceptance steps.
